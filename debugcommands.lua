...
function d_createscrapbookdata()
        ...
        for i, data in pairs(scrapbookprefabs) do
           ...
           -- str = str .. 
            ...
            if t.components.sanityaura and getsanity(t) then
                 f:write(' sanityaura='..  getsanity(t) ..',')
            end

            --------------------------------------
            ---------------------------------- HEALTH
            if t.components.health then
                f:write(' health='..  t.components.health.maxhealth ..',')
            end

            --------------------------------------
            ---------------------------------- DAMAGE
            if t.scrapbook_damage then
                f:write(' damage='..  t.scrapbook_damage ..',')
            elseif  t.components.combat and t.components.combat.defaultdamage then
                f:write(' damage='..  t.components.combat.defaultdamage ..',')
            end            

            --------------------------------------
            ---------------------------------- STACK
            if t.components.stackable  then
                f:write(' stacksize='..  t.components.stackable.maxsize ..',')
            end

            --------------------------------------
            ---------------------------------- FOOD
            if t.components.edible  then
                local substr = ' hungervalue='..  t.components.edible.hungervalue ..','
                substr= substr..' healthvalue='..  t.components.edible.healthvalue ..','
                substr= substr..' sanityvalue='..  t.components.edible.sanityvalue ..','
                f:write(substr)
            end

            if t.components.edible and t.components.edible.foodtype  then                
                 f:write(' foodtype="'..  t.components.edible.foodtype ..'",')
            end
            ...
            if t.components.weapon or t.scrapbook_weapondamage then
                if t.prefab == "bomb_lunarplant" then
                    ...
                    if planardamage then
                        f:write(' planardamage='.. planardamage ..',')        
                    end

                    if t.components.weapon and t.components.weapon.hitrange then
                         f:write(' weaponrange='..  t.components.weapon.hitrange ..',')
                    end
                end
            end
            ...
            if t.components.armor then                
                f:write(' armor='.. t.components.armor.maxcondition ..',')
                f:write(' absorb_percent='.. t.components.armor.absorb_percent ..',')
                if t.components.planardefense then
                    f:write(' armor_planardefence='.. t.components.planardefense.basedefense ..',')
                end                
            end
            ...
            if t.components.finiteuses  then                
                f:write(' finiteuses='..  t.components.finiteuses.total ..',')
            end            
            if t.components.tool then
                     f:write(' toolactions={')
            ...
            if t.scrapbook_scale then
                f:write(' scrapbook_scale='.. t.scrapbook_scale ..',')  
            end

            if t.scrapbook_setanim then
                f:write(' scrapbook_setanim='.. t.scrapbook_setanim ..',')  
            end

            if t.scrapbook_overridebuild then
                 f:write(' scrapbook_overridebuild="'.. t.scrapbook_overridebuild ..'",')  
            end
    ...
    print("PREFAB:",t.prefab,i)

            f:write(' build="'..  build ..'",')
            f:write(' bank="'..  t.AnimState:GetCurrentBankName() ..'",')
            f:write(' anim="'..  anim ..'",')
            ...
            if t.prefab == "robin" then                
                f:write(' animoffsety=' .. -8 ..', ')
            end
            if t.prefab == "robin_winter" then            
                f:write(' animoffsety=' .. -15 ..', ')
                f:write(' animoffsetbgy=' .. 15 ..', ')
            end
            if t.prefab == "friendlyfruitfly" then            
                f:write(' animoffsety=' .. 100 ..', ')
            end
            ...
            if t.prefab == "minotaur" then                
                f:write(' animoffsetx=' .. 5 ..', ')
            end            
            if t.prefab == "lordfruitfly" then
                f:write(' animoffsety=' .. 70 ..', ')
            end
            if t.prefab == "moonbutterfly" then
                f:write(' animoffsetx=' .. 15 ..', ')
            end
            if t.prefab == "bee" then
                f:write(' animoffsety=' .. 150 ..', ')
            end
            ...
            if t.prefab == "moon_altar_crown" then                
                f:write(' animoffsety=' .. -20 ..', ')
                f:write(' animoffsetx=' .. 25 ..', ')
                f:write(' animoffsetbgy=' .. 30 ..', ')
            end
            if t.prefab == "shroomcake" then
                f:write(' animoffsety=' .. -20 ..', ')
                f:write(' animoffsetbgy=' .. 25 ..', ')
            end
            ...
            if t.prefab == "bernie_active" then
                f:write(' animoffsety=' .. 60 ..', ')
                f:write(' animoffsetbgy=' .. -50 ..', ')
            end 
            ...
            -----------------------------------------
            ------------------------------------ WATERPROOFER
            if t.components.waterproofer then
                f:write(' waterproofer=' .. t.components.waterproofer:GetEffectiveness() ..', ')
            end

            -----------------------------------------
            ------------------------------------ INSULATOR
            if t.components.insulator then
                f:write(' insulator=' .. t.components.insulator:GetInsulation() ..', ')
                f:write(' insulator_type="' .. t.components.insulator.type ..'", ')
            end  

            -----------------------------------------
            ------------------------------------ DAPPERNESS
            if t.components.equippable and t.components.equippable.dapperness then
                f:write(' dapperness=' .. t.components.equippable.dapperness ..', ')
            end

            -----------------------------------------
            ------------------------------------ FUELED
            if t.components.fueled then
                f:write(' fueledmax=' .. t.components.fueled.maxfuel ..', ')
                f:write(' fueledrate=' .. t.components.fueled.rate ..', ')
                f:write(' fueledtype1="' .. t.components.fueled.fueltype ..'", ')
                if t.components.fueled.secondaryfueltype then
                    f:write(' fueledtype2="' .. t.components.fueled.secondaryfueltype ..'", ')
                end
            end          
            ...
            if t.components.fuel then           
                f:write(' fueltype="' .. t.components.fuel.fueltype ..'", ')
                f:write(' fuelvalue=' .. t.components.fuel.fuelvalue ..', ')
            end     

            -----------------------------------------
            ------------------------------------ FUELED
            if t.components.perishable then
                f:write(' perishable=' .. t.components.perishable.perishtime ..', ')
            end     

            -----------------------------------------
            ------------------------------------ FUELED
            if t:HasTag("lightbattery") then
                f:write(' lightbattery=true,')
            end     

            -----------------------------------------
            ----------------------------------- CRAFTING ICON

            for recipeprefab,recipe in pairs(AllRecipes)do
                if t.prefab == recipeprefab then

                    if recipe.builder_tag then
                        local character = nil

                        if recipe.builder_tag == "spiderwhisperer" then
                            character = "webber"
                        elseif recipe.builder_tag == "plantkin" then
                            character = "wormwood"
                        elseif recipe.builder_tag == "shadowmagic" then
                            character = "waxwell"
                        elseif recipe.builder_tag == "handyperson" then
                            character = "winona"
                        elseif recipe.builder_tag == "handyperson" then
                            character = "winona"
                        elseif recipe.builder_tag == "masterchef" or recipe.builder_tag == "professionalchef" then
                            character = "warly"
                        elseif recipe.builder_tag == "merm_builder" then
                            character = "wurt"
                        elseif recipe.builder_tag == "elixirbrewer" or  recipe.builder_tag == "ghostlyfriend" then
                            character = "wendy"
                        elseif recipe.builder_tag == "werehuman" then
                            character = "woodie"
                        elseif recipe.builder_tag == "valkyrie" or recipe.builder_tag == "battlesinger" then
                            character = "wathgrithr"
                        elseif recipe.builder_tag == "pebblemaker" or recipe.builder_tag == "pinetreepioneer" then
                            character = "walter"   
                        elseif recipe.builder_tag == "strongman" then
                            character = "wolfgang" 
                        elseif recipe.builder_tag == "bookbuilder" then
                            character = "wickerbottom" 
                        elseif recipe.builder_tag == "clockmaker" then
                            character = "wanda"                             
                        elseif recipe.builder_tag == "balloonomancer" then
                            character = "wes"  
                        elseif recipe.builder_tag == "upgrademoduleowner" then
                            character = "wx78"                              
                        elseif recipe.builder_tag == "alchemist" 
                            or recipe.builder_tag == "gem_alchemistI" 
                            or recipe.builder_tag == "gem_alchemistII" 
                            or recipe.builder_tag == "gem_alchemistIII" 
                            or recipe.builder_tag == "ore_alchemistI" 
                            or recipe.builder_tag == "ore_alchemistII" 
                            or recipe.builder_tag == "ore_alchemistIII"
                            or recipe.builder_tag == "ick_alchemistI"
                            or recipe.builder_tag == "ick_alchemistII"
                            or recipe.builder_tag == "ick_alchemistIII" 
                            or recipe.builder_tag == "skill_wilson_allegiance_shadow" 
                            or recipe.builder_tag == "skill_wilson_allegiance_lunar" then
                            character = "wilson" 
                        end                      

                        if character then
                            f:write(' craftingprefab="' .. character ..'", ')
                        end
            ...
            local deps = Prefabs[i].deps

            if t.scrapbook_deps then
                deps = {}
                for d,dep in ipairs(t.scrapbook_deps)do
            ...
            for recipeprefab,recipe in pairs(AllRecipes)do
                if t.prefab == recipeprefab then
                    ...
                    local ingredients ={}
                    for ing, ingdata in ipairs(recipe.ingredients)do
                        ingredients[ingdata.type] = true
                    end

                    for ing, bool in pairs(ingredients)do
                        local found =false
                        for z,zdep in ipairs(deps)do
                            if zdep == ing then
                                found = true
                            end
                        end
