...
local MiasmaOver = Class(Widget, function(self, owner, cloudlayer)
	self.owner = owner
	Widget._ctor(self, "MiasmaOver")
	...
	self.bg:GetAnimState():SetBank("miasma_over")
	self.bg:GetAnimState():SetBuild("miasma_over")
	...
	if owner ~= nil then
		...
		self.inst:ListenForEvent("miasmalevel", function(owner, data)
		...
		if owner.IsInMiasma ~= nil and owner:IsInMiasma() then
			self:FadeTo(1, true)
		end
	end
end)

function MiasmaOver:BlindTo(blindto, instant)
	...
	if self.blindto ~= blindto then
		...
		if self.clouds.shown then
			--TheFocalPoint.SoundEmitter:SetParameter("sandstorm", "intensity", blindto < 1 and 0 or .5)
		end
	end
end

function MiasmaOver:FadeTo(fadeto, instant)
	if self.owner and not self.owner:IsInMiasma() then
		fadeto = 0
	end
...
function MiasmaOver:ApplyLevels()
	self.alpha = math.max(0, self.fade * 1.5 - .5) * self.blind
	if self.alpha > 0 then
		self.bg:GetAnimState():SetMultColour(1, 1, 1, self.alpha)
		self.letterbox:SetMultColour(BGCOLOR[1], BGCOLOR[2], BGCOLOR[3], BGCOLOR[4] * self.alpha)
	...
	if self.fade > 0 then
		...
		self.clouds:GetAnimState():SetMultColour(1, 1, 1, f * .5)
		if not self.clouds.shown then
			self.clouds:Show()
	...
	elseif self.clouds.shown then
		self.clouds:Hide()
		--TheFocalPoint.SoundEmitter:KillSound("sandstorm")
	end
end

function MiasmaOver:OnUpdate(dt)
