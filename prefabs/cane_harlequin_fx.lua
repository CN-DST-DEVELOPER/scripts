...
local COLOUR_ENVELOPE_NAME = "cane_confetti_colourenvelope"
local SCALE_ENVELOPE_NAME = "cane_confetti_scaleenvelope"
...
--------------------------------------------------------------------------
local MAX_LIFETIME = 2.3

local function emit_confetti_fn(effect, i, spark_sphere_emitter)
    local lifetime = MAX_LIFETIME * (.5 + UnitRand() * .5)
    local px, py, pz = spark_sphere_emitter()
    local vx, vy, vz = px * 0.75, py * 0.75, pz * 0.75
    vy = vy + 0.3 --apply upward force to all confetti
    ...
    EmitterManager:AddEmitter(inst, nil, function()
        ...
        for i = 0,num_emitters do
            ...
            while num_to_emit > 0 do
                emit_confetti_fn(effect, i, sphere_emitter)
...
return Prefab("cane_harlequin_fx", fn, assets)
