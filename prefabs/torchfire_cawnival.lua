...
local COLOUR_ENVELOPE_NAME_SMOKE = "torch_cawnival_colourenvelope_smoke"
local SCALE_ENVELOPE_NAME_SMOKE = "torch_cawnival_scaleenvelope_smoke"
local COLOUR_ENVELOPE_NAME = "torch_cawnival_colourenvelope"
local SCALE_ENVELOPE_NAME = "torch_cawnival_scaleenvelope"
local COLOUR_ENVELOPE_NAME_EMBER = "torch_cawnival_colourenvelope_ember"
local SCALE_ENVELOPE_NAME_EMBER = "torch_cawnival_scaleenvelope_ember"
...
local function InitEnvelope()
    ...
    EnvelopeManager:AddColourEnvelope(
        ...
        {
            { 0,   IntColour(200, 200, 200, 25) },
            { .5,  IntColour(200, 200, 0, 255) },
            { 1,   IntColour(0, 0, 0, 0) },
        }
    )

    local fire_max_scale = .09
    EnvelopeManager:AddVector2Envelope(
        SCALE_ENVELOPE_NAME,
        {
            { 0,    { fire_max_scale * .75, fire_max_scale * .75 } },
            { 1,    { fire_max_scale * .25, fire_max_scale * .25 } },
        }
    )
    ...
    EnvelopeManager:AddColourEnvelope(
        COLOUR_ENVELOPE_NAME_EMBER,
        {
            ...
            { .3,   IntColour(200, 200, 100, 255) },
            { .6,   IntColour(200, 200, 0, 255) },
            { 1,    IntColour(0, 0, 0, 0) },
        }
    )

    local ember_max_scale = .5
    EnvelopeManager:AddVector2Envelope(
...
local function emit_smoke_fn(effect, sphere_emitter)
    local vx, vy, vz = .01 * UnitRand(), .08, .01 * UnitRand()
...
local function emit_fire_fn(effect, sphere_emitter)
    ...
    local lifetime = FIRE_MAX_LIFETIME * (.75 + UnitRand() * .25)
    local px, py, pz = sphere_emitter()
    local uv_offset = math.random(0, 3) * .25
...
local function emit_ember_fn(effect, sphere_emitter)
    local vx, vy, vz = .2 * UnitRand(), .2 * UnitRand(), .2 * UnitRand()
...
local function common_postinit(inst)
    ...
    local ember_num_particles_to_emit = 2
    ...
    EmitterManager:AddEmitter(inst, nil, function()
        ...
        if ember_time_to_emit < 0 then
            ...
            ember_time_to_emit = (math.random() * .2)
        end
...
return MakeTorchFire("torchfire_cawnival", assets, nil, common_postinit, master_postinit)
