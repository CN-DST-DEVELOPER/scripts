local assets =
{
    Asset("ANIM", "anim/vault_key.zip"),
}

local assets_icon =
{
    Asset("MINIMAP_IMAGE", "vault_key"),
}
...
local function fn()
    ...
    inst.AnimState:SetBank("vault_key")
    inst.AnimState:SetBuild("vault_key")
    ...
    inst.icon = SpawnPrefab("vault_key_icon")
    inst.icon.entity:SetParent(inst.entity)
...
local function iconfn()
    ...
    inst.MiniMapEntity:SetIcon("vault_key.png")
...
return Prefab("vault_key", fn, assets, prefabs),
    Prefab("vault_key_icon", iconfn, assets_icon, prefabs_icon)
