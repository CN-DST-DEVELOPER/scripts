...
local function SpawnGhostsOnGraves(graves, scenariorunner, player)
...
local function OnGraveDug(inst, mound, scenariorunner, data)
	local player = data and data.worker
...
local function OnLoad(inst, scenariorunner)
	...
	for k,v in pairs(inst.moundlist) do
		if v.components.workable then
			v.scene_workfinishedfn = function(oninst, data) OnGraveDug(inst, v, scenariorunner, data) end
